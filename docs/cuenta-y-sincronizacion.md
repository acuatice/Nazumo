...
4. En **Authentication → Providers → Email**, activa la confirmación de correo. En las plantillas usa `{{ .RedirectTo }}` y el hash de token:
   ...
   La app solicita `/auth/confirm` o `/auth/recover` como destino; el callback verifica el `token_hash` (y también acepta el código PKCE `code`). El SMTP predeterminado limita los correos; configura SMTP propio antes de abrir el registro al público.
