...
export function Navigation() {
  ...
  return <>
    <header className="sticky top-0 z-30 -mx-4 flex h-[4.25rem] items-center justify-between border-b border-black/[.035] bg-[var(--background)]/90 px-4 backdrop-blur-xl min-[375px]:-mx-5 min-[375px]:px-5 sm:-mx-8 sm:px-8 lg:-mx-10 lg:h-24 lg:px-10">
      ...
      <Link href="/cuenta" aria-label={learnerName ? `Cuenta de ${learnerName}` : "Cuenta"} className="flex min-h-10 items-center justify-center gap-2 rounded-full bg-[var(--nazumo-lavender)] px-2 text-xs font-bold text-[var(--nazumo-purple)] ring-2 ring-white transition hover:scale-105 min-[375px]:px-3"><span className="flex size-8 items-center justify-center overflow-hidden rounded-full bg-white/70">{learnerName ? learnerName.slice(0, 1).toUpperCase() : "日"}</span><span className="sr-only min-[375px]:not-sr-only">Cuenta</span></Link>
    </header>
